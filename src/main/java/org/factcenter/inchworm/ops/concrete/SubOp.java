...
import org.factcenter.fastgc.inchworm.SubOpInchworm;
...
public class SubOp extends ConcreteCommon implements OpAction {
	/*-
	 * ----------------------------------------------------------------
	 *                    Constructor(s). 
	 * ----------------------------------------------------------------
    ...
    public SubOp() { this(null);  }

	/**
	 *  Constructs a new {@code SubOp} object.
     *
     *  widthOverride allows the caller to override the default bit width ({@link VMState#getWordSize()} with
     *  a custom width.
	 */
	public SubOp(final VariableWidth widthOverride) {
		/*-
        ...
        super(new CircuitWrapperFactory() {
            ...
            public InchwormOpCommon createNewCircuitWrapper(boolean isServer, Random rand, int labelBitLength, int bitWidth, Channel yaoChannel, OTExtender otExtender) {
                return new SubOpInchworm(isServer, rand, labelBitLength,
    ...
    public BitMatrix[] doOp(VMState state, BitMatrix... inputs) throws IOException {
        ...
        BitMatrix rValue = new BitMatrix(opWidth + 4); // todo: shouldn't this be 4 (also in add?)
        ...
        try {
            /*-
             * 1) Set shared data into the sub op and run.
             */
