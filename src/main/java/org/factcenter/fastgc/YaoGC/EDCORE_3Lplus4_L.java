...
public class EDCORE_3Lplus4_L extends CompositeCircuit {
	...
	public EDCORE_3Lplus4_L(CircuitGlobals globals, int l) {
		super(globals, 3*l+4, l, 5, "EDCORE_" + (3*l+4) + "_" + l);
		L = l;
	}
	...
	protected void createAllSubCircuits(boolean isForGarbling) {
		...
		subCircuits[MINB]  = new MIN_2L_L(globals, L+1);   // because ADD1 can have (L+1)-bit output
		subCircuits[ADD1A] = new ADD1_Lplus1_Lplus1(globals, L);
		subCircuits[ADD1B] = new ADD1_Lplus1_Lplus1(globals, L);
		subCircuits[EDT]   = new EDT_4_1(globals);
	}
